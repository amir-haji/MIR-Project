...
class ClusteringUtils:
    ...
    def visualize_kmeans_clustering_wandb(self, data, n_clusters, document_labels, project_name, run_name):
        ...
        # Plot the clusters
        os.makedirs(f'clustering_plots/{project_name}/{run_name}', exist_ok=True)
        figure = plt.figure()
        for k in groups:
        ...
        plt.legend(loc = 'best')
        plt.savefig(f'clustering_plots/{project_name}/{run_name}/Clustering_{n_clusters}.png')
    ...
    def wandb_plot_hierarchical_clustering_dendrogram(self, data, linkage_method, project_name, run_name):
        ...
        plt.savefig(f'clustering_plots/{project_name}/{run_name}/Hierarchical Clustering {linkage_method}.png')
    ...
    def plot_kmeans_cluster_scores(self, embeddings: List, true_labels: List, k_values: List[int], project_name=None,
        ...
        plt.title('Silhoueete scores')
        
        plt.subplot(2, 1, 2)
        plt.plot(k_values, purity_scores)
        plt.title('Purity scores')
        plt.savefig(f'clustering_plots/{project_name}/{run_name}/metrics.png')
        plt.close()
    ...
    def visualize_elbow_method_wcss(self, embeddings: List, k_values: List[int], project_name: str, run_name: str):
        ...
        plt.savefig(f'clustering_plots/{project_name}/{run_name}/Elbow Method.png')
        plt.close()
        # Log the plot to wandb
