...
class Scorer:
    def __init__(self, index, number_of_documents):
        ...
        for term, posting in index.items():
            self.T += sum(list(posting.values()))
    ...
    def compute_scores_with_unigram_model(
        ...
        for document_id in documents:
            document_scores[document_id] = self.compute_score_with_unigram_model(query, document_id, smoothing_method, document_lengths, alpha, lamda)
        ...
        for term in query:
            ...
            if smoothing_method == 'naive':
                ...
                if smoothing_method == 'bayes':
                    score *= ((tf + alpha * (cf / self.T)) / (document_lengths[document_id] + alpha))
                else:
                    score *= (lamda * (tf / document_lengths[document_id]) + (1 - lamda) * (cf / self.T))
